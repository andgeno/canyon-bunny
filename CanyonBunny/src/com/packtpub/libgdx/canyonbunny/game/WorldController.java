...
public class WorldController extends InputAdapter implements Disposable {
	...
	private boolean helpTiltWasVisible;
	private boolean helpTouchWasVisible;
	private boolean helpFlyWasVisible;
	...
	private void testCollisions () {
		...
		if (level.bunnyHead.position.x >= 3 && level.bunnyHead.position.x <= 4) {
			showHelpTouch(false);
		} else if (level.bunnyHead.position.x >= 20 && level.bunnyHead.position.x <= 21) {
			showHelpFly(false);
		}
	}
...
// showHelpTouch(false);
	...
	private void showHelpTilt (boolean instantaneous) {
		...
		float yDst = -2;
		float duration = 3;
		Interpolation easing = Interpolation.elasticOut;
		if (instantaneous) duration = 0;
		level.helpTilt.moveBy(xSrc, ySrc, xDst, yDst, duration, easing);
	}

	private void showHelpTouch (boolean instantaneous) {
	...
	private void showHelpFly (boolean instantaneous) {
