...
public class MenuScreen extends AbstractGameScreen {
	...
	private Image imgBookCover;
	private Label lblBookMusicCredits;
	...
	private void rebuildStage () {
		...
		Actor layerBook = buildBookLayer();
		Actor layerMusicCredits = buildMusicCreditsLayer();
...
// stack.add(layerBook);
	...
	private Table buildLogosLayer () {
		...
		imgInfo.addListener(new ClickListener() {
			@Override
			public void clicked (InputEvent event, float x, float y) {
				Gdx.net.openURI("http://www.gamerald.com/");
			}
		});
	...
	private Actor buildBookLayer () {
		float delayStart = 10;
		float delayLoopMin = 3;
		float delayLoopMax = 6;
		imgBookCover = new Image(skinCanyonBunny, "book-cover");
		imgBookCover.setOrigin(imgBookCover.getWidth() / 2, imgBookCover.getHeight() / 2);

		float scaleImage = 1;
		float scaleUpSize = scaleImage * 1.25f;
		float scaleUpDur = 0.025f;
		float scaleDownDur = 0.25f;

		imgBookCover.addAction(sequence(

			moveTo(-imgBookCover.getWidth(), (Constants.VIEWPORT_GUI_HEIGHT - imgBookCover.getHeight()) / 2),
			...
			parallel(moveBy(imgBookCover.getWidth() * 1.2f, 0, 1, Interpolation.swingOut),
			...
			rotateBy(360 * 2, 1f, Interpolation.swing),

			rotateTo(0)

			))));

		imgBookCover.addListener(new ClickListener() {
			@Override
			public void clicked (InputEvent event, float x, float y) {
	...
	public Actor buildMusicCreditsLayer () {
		float delay = 13;
		float moveDistance = 10;

		Label lbl = new Label("Music by Klaus \"keith303\" Spang", skinLibgdx.get(LabelStyle.class));
		lbl.setPosition(380, 0);

		lbl.addAction(sequence(moveBy(0, -50), alpha(0),

		delay(delay),

		parallel(moveBy(0, +50 + moveDistance, 1, Interpolation.linear), alpha(0.8f, 1.5f))
